...
int find_min(BST_Node *root)
{
    ...
    if (root->left==NULL) return root->bar;
...
{
    if (root==NULL) return 0;
    if (root->right ==NULL) return root->bar;
    return find_max(root->right);
}

void reverse(BST_Node *root,int min_bar, int max_bar)
{
    if (root==NULL )return;
    ...
    reverse(root->left,min_bar,max_bar);
    reverse(root->right,min_bar,max_bar);
...
BST_Node *reverseSong(BST_Node *root)
{
    ...
    int min_bar=find_min(root);
    int max_bar=find_max(root);
    reverse(root,min_bar,max_bar);
    return root;
...
void harmony(BST_Node *root, BST_Node *now, int semitones, double time_shift){
    if (now == NULL) {
        return;
    }
    if (time_shift > 0) {
        harmony(root,now->right, semitones, time_shift);
        harmony(root,now->left, semitones, time_shift);
    } else {
        harmony(root,now->right, semitones, time_shift);
        harmony(root,now->left, semitones, time_shift);
    }
    int j = find_freq(now->freq);
    if (j == -1) {
        return;
    }
    int new_j = j + semitones;
    double new_index = now->index + time_shift;
    if (new_j >= 0 && new_j <= 99&& new_index >= 0.0 && new_index < 1.0) {
        double new_freq = note_freq[new_j];
        BST_Node *new_node = newBST_Node(new_freq, now->bar, new_index);
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
     ...
     */
    harmony(root,root,semitones,time_shift);
    return root;
}
